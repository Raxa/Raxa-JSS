...
DocPanel = Ext.extend(Ext.Panel, {
    ...
    scrollToMember : function(member){
        ...
        if (!el) {
            var res = this.findMember(member);
            el = Ext.fly(res);
            member = res.id;
        };
        ...
        if(el){
            ...
            this.body.scrollTo('top', top-25, {duration:0.75, callback: this.hlMember.createDelegate(this, [member, (res) ? true : false])});
        }
    },
    
    findMember: function(member) {
        var els  = Ext.getBody().select('a'),
            clss = [];
        
        for (var i = 0; i < els.elements.length; i++) {
            if (els.elements[i].id.match(member)) {
                return els.elements[i];
            };
        }
    },
    ...
    hlMember : function(member, dontAddCls){
        console.log((dontAddCls) ? member : this.cclass + '-' + member);
        var el = Ext.fly((dontAddCls) ? member : this.cclass + '-' + member);
        if(el){
            if (tr = el.up('tr')) {
                tr.highlight('#cadaf9');
            }
