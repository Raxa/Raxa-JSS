		...
		try {
			for (int i = 0; i < pathElements.length; ++i) {
				...
				if (pathElements[i].indexOf("[") > 0) {
					property = pathElements[i].substring(0, pathElements[i]
							.indexOf("["));
					index = Integer.valueOf(pathElements[i].substring(
							pathElements[i].indexOf("[") + 1, pathElements[i]
									.indexOf("]")));
				} else {
			...
			throw new RuntimeException("Failed to get path "
					+ OpenmrsUtil.join(Arrays.asList(pathElements), " . "), ex);
		}
	}
}
