...
@Resource("fieldType")
@Handler(supports = FieldType.class, order = 0)
public class FieldTypeResource
	 ...
	 */
	@Override
	public List<FieldType> doGetAll(RequestContext context) {
		return Context.getFormService().getAllFieldTypes(false);
	}
	...
	protected FieldType save(FieldType delegate) {
		Context.getFormService().saveFieldType(delegate);
		return delegate;
	}
	...
	/**
	 * Gets the display string for a field type.
	 * 
	 * @param ft the fieldType.
	 * @return attribute type + value (for concise display purposes)
	 */
	public String getDisplayString(FieldType ft) {
		return ft.getName();
	}
